# -*- coding: utf-8 -*-
"""
Created on Wed May  9 16:57:02 2018
...
with tf.Session() as sess:
    sess.run(init)
    for epoch in range(traini_epochs):
    ...
    print('optimisation over')
    correct_prediction = tf.equal(tf.argmax(logits, 1), tf.argmax(y, 1))
    accu = tf.reduce_mean(tf.cast(correct_prediction, "float"))
    test_size = 999
    print("Accuracy:", accu.eval({x: mnist.test.images[:test_size], y: mnist.test.labels[:test_size]}))
